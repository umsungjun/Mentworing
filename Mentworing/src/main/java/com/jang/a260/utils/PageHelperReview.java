...
public class PageHelperReview {
	...
	public StringBuffer getPageUrl(SearchVO searchVO, int totalRow) {
		...
		System.out.println("리뷰 갯페이지 유얼엘222");
		...
		if(currentBlock > 1){
			pageHtml.append("<span><a href=\"../board/Mentor_Review?page=" + (currentPage-1) + smsg+ "\"><이전></a>&nbsp;&nbsp;");
		}else{
		...
		pageHtml.append("&nbsp;</strong>");
			...
			pageHtml.append(".&nbsp;<a href=\"../board/Mentor_Review?page=" +i+smsg+ "\" class=\"page\">" + i + "</a>&nbsp;");
		}
		}
		//현재 블록 안이 전체 블럭 보다 작으면 [다음] 표시
		if(currentBlock < totalBlock){
			pageHtml.append(".&nbsp;&nbsp;<a href=\"../board/Mentor_Review?page=" + (currentPage+1) +smsg+ "\"><다음></a></span>");
		}else{
		pageHtml.append(".</span>");
		}
		return pageHtml; 
